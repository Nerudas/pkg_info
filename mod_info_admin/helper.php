...
class ModInfoAdminHelper
{
	...
	public static function getAjax()
	{
		if ($params = self::getModuleParams(Factory::getApplication()->input->get('module_id', 0)))
		{
			...
			BaseDatabaseModel::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_info/models', 'PagesModel');
			$model = BaseDatabaseModel::getInstance('Items', 'InfoModel', array('ignore_request' => false));
			$app->setUserState('com_info.items.list', array(
				'fullordering' => $params->get('ordering', 'i.created DESC'),
				'limit'        => $params->get('limit', 5)
			));

			$items = $model->getItems();
			$app->setUserState('com_info.items.list', '');
		...
		throw new Exception(Text::_('MOD_INFO_ADMIN_ERROR_MODULE_NOT_FOUND'), 404);
	}
