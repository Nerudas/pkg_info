...
(function ($) {
	$(document).ready(function () {
		$('[data-mod-info-admin]').each(function () {
			var block = $(this),
				reload = $(block).find('[data-mod-info-admin-reload]');
			getItems(block);
		...
		function getItems(block) {
			var loading = block.find('.loading'),
				...
				module_id = $(block).data('mod-info-admin');
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: 'index.php?option=com_ajax&module=info_admin&format=json',
