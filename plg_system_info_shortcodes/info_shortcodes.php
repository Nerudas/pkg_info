<?php
/**
 * @package    System - Information Shortcodes Plugin
 * @version    1.2.5
...
class plgSystemInfo_Shortcodes extends CMSPlugin
{
	...
	public function onAfterRender()
	{
		...
		if ($app->isSite())
		{
			...
			preg_match_all('/{info id="(.*?)" layout="(.*?)"}/', $body, $matches);
			...
			if (!empty($ids))
			{
				...
				$language->load('com_info', JPATH_SITE, $language->getTag());

				BaseDatabaseModel::addIncludePath(JPATH_SITE . '/components/com_info/models', 'InfoModel');
				...
				foreach ($shortcodes as $key => $shortcode)
				{
					...
					$replace = ($id && $layout && $item) ?
						LayoutHelper::render('components.com_info.shortcodes.' . $layout, $item) : '';
